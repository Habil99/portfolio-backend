...
class BannerValidator {
    async validateCreateBody(req, _res, next) {
        // validate req body
        ...
        await (0, class_validator_1.validate)(bannerDto).then(errors => {
            if (errors.length > 0) {
                throw http_exception_1.HttpException.badRequest('Bad Request', (0, helpers_1.normalizeValidatorErrors)(errors));
            }
            return next();
        });
    }
    async validateUpdateBody(req, _res, next) {
        ...
        if (!updateBannerDto.username && !updateBannerDto.title && !updateBannerDto.description && !updateBannerDto.isActive) {
            throw http_exception_1.HttpException.badRequest('Bad Request', 'At least one field is required');
        }
        await (0, class_validator_1.validate)(updateBannerDto).then(errors => {
            if (errors.length > 0) {
                throw http_exception_1.HttpException.badRequest('Bad Request', (0, helpers_1.normalizeValidatorErrors)(errors));
            }
            return next();
